...
func FuzzSetJWTCookie(f *testing.F) {
	...
	f.Add([]byte("secret-key"), "user_id", "12345", "role", "admin")
	f.Add([]byte("another-key"), "email", "test@example.com", "name", "Test User")
	f.Add([]byte("short"), "a", "b", "c", "d")
	f.Add([]byte(""), "key", "value", "", "")

	f.Fuzz(func(t *testing.T, secretKey []byte, key1, val1, key2, val2 string) {
		// Skip if secret key is empty as it would cause issues
		if len(secretKey) == 0 {
			return
		}
...
func FuzzGetClaimsOfValid(f *testing.F) {
	// Seed corpus with some basic examples
	f.Add([]byte("secret-key"), "jwt_token", "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyfQ.SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_adQssw5c")
	f.Add([]byte("another-key"), "custom_token", "invalid.token.value")
	f.Add([]byte("test"), "jwt_token", "")
	f.Add([]byte(""), "jwt_token", "some-value")

	f.Fuzz(func(t *testing.T, secretKey []byte, cookieName, tokenValue string) {
		// Skip if secret key is empty
		if len(secretKey) == 0 {
			return
		}
...
func FuzzRoundTrip(f *testing.F) {
	// Seed corpus
	f.Add([]byte("secret-key"), "user_id", "12345")
	f.Add([]byte("another-secret"), "email", "test@example.com")
	f.Add([]byte("x"), "a", "b")

	f.Fuzz(func(t *testing.T, secretKey []byte, claimKey, claimValue string) {
		// Skip if secret key is empty
		if len(secretKey) == 0 {
			return
		}
