...
func (cm *CookieManager) SetJWTCookie(w http.ResponseWriter, r *http.Request, customClaims map[string]string) error {
	...
	// Verify all claims are valid UTF-8 and use the allowed character set.
	for k, v := range customClaims {
		// check key and value are valid UTF-8 and alphanumeric (including +, #, -, _)
		if !isAlphanumericUtf8(k) {
			return fmt.Errorf("claim key %q must be valid UTF-8 and contain only [A-Za-z0-9_+#-]", k)
		}
		if !isAlphanumericUtf8(v) {
			return fmt.Errorf("claim value for key %q must be valid UTF-8 and contain only [A-Za-z0-9_+#-]", k)
		}
	}
